...
const PdfViewer = ({ fileUrl }) => {
  ...
  // данные из state
  const learningStudentSubjectId = location.state?.learningStudentSubjectId;
  ...
  const handleFinish = async () => {
    if (!lessonId || !learningStudentSubjectId) {
    ...
    try {
      const res = await ApiCall(
        `/api/v1/learning-student-subject/add-read-lesson/${lessonId}/${learningStudentSubjectId}`,
        "POST"
      );

      if (!res.error) {
        toast.success("✅ Yakunlandi!", { autoClose: 1500 });
        setTimeout(() => {
          if (curriculumSubjectId) {
            navigate(`/student/media-lessons/${curriculumSubjectId}`, {
              replace: true,
              state: { requiredLessons, debtsId, refresh: true },
            });
          } else {
        ...
        // ⚡ Урок уже был завершён → отправляем на student/default
        toast.info("ℹ️ Siz darsni tugallagansiz", { autoClose: 1500 });
        setTimeout(() => {
          navigate("/student/debts", { replace: true });
        }, 1600);
      }
    } catch (err) {
