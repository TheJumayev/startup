  ...
  useEffect(() => {
    const loadContracts = async () => {
      for (let item of allData) {
        if (item.student?.studentIdNumber) {
          await getContract(item.student.studentIdNumber);
        }
      }
    };
      ...
      <div className="mb-8">
        <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-center">
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-xl bg-green-600 px-5 py-3 text-white shadow-lg transition-all duration-300 hover:bg-green-700"
          >
