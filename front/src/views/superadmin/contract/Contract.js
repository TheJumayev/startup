...
const Contract = () => {
  ...
  const fetchContractAmount = async () => {
    try {
      ...
      console.log("Foiz hali kiritilmagan");
    }
  };

  // ==== INPUT O‘ZGARISHINI TEKSHIRISH ====
  const handleAmountChange = (e) => {
    const val = e.target.value;
    if (val === "" || (val >= 0 && val <= 100)) {
      setAmount(val);
    } else {
      toast.warn("Foiz 10 dan kichik yoki 100 dan katta bo‘lmasin!");
    }
  };
  ...
  const handleAmountSubmit = async (e) => {
    ...
    if (isNaN(num) || num < 0 || num > 100) {
      toast.error("Iltimos, 10 dan 100 gacha bo‘lgan butun son kiriting!");
      return;
    }
    ...
    try {
      ...
      toast.success("Kontrakt foizi muvaffaqiyatli saqlandi!");
      await fetchContractAmount();
  ...
  const closeAmountModal = () => setAmountModal(false);
        ...
        <div className="flex items-center justify-between">
          ...
          <div>
            <button
              ...
              className="flex items-center gap-2 rounded-lg bg-amber-500 py-2 px-5 font-medium text-white shadow-sm transition-all hover:bg-amber-600"
            >
              Foizni {currentAmount ? "o‘zgartirish" : "kiritish"}
            </button>

            {/* 🔹 Oxirgi foizni ko‘rsatish qismi */}
            {currentAmount && (
              <div className="mt-3 ml-2 text-sm text-gray-700">
                <p>
                  <span className="font-semibold text-blue-600">
                    Oxirgi foiz: {currentAmount.amount}%
                  </span>{" "}
                  <span className="text-xs text-gray-500">
                    ({new Date(currentAmount.createdAt).toLocaleString("uz-UZ")}
                    )
                  </span>
                </p>
              </div>
      ...
      >
        <div className="p-5">
          <h2 className="mb-4 text-2xl font-bold text-gray-800">
            Kontrakt foizini {currentAmount ? "o‘zgartirish" : "kiritish"}
          </h2>
          ...
          <form onSubmit={handleAmountSubmit}>
            <div className="mb-4">
              ...
              <input
                ...
                onChange={(e) => {
                  const val = e.target.value;
                  // faqat raqamlarni kiritish va oraliqni tekshirish
                  if (/^\d*$/.test(val)) {
                    setAmount(val);
                  }
                }}
                onBlur={() => {
                  ...
                  if (num && (num <= 0 || num > 100)) {
                    toast.warning(
                      "Foiz 10 dan kichik yoki 100 dan katta bo‘lmasin!"
                    );
                  }
                }}
                className="w-full rounded-lg border border-gray-300 py-2 px-3 text-lg text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <div className="mt-5 flex justify-evenly">
              <button
                type="submit"
                disabled={isLoading}
                className="rounded-lg bg-blue-600 py-2 px-6 font-semibold text-white hover:bg-blue-700"
              >
              ...
              <button
                ...
                className="rounded-lg bg-gray-400 py-2 px-6 font-semibold text-white hover:bg-gray-500"
              >
                Bekor qilish
              </button>
            </div>
