...
function ScoreStudent() {
  ...
  const { isKursIshi } = location.state || {};
  ...
  const getAdmin = async () => {
    try {
      ...
      console.log("Decoded admin:", response.data);
  ...
  const handlePassedChange = async (scoreId, value) => {
    ...
    try {
      await ApiCall(`/api/v1/score-sheet/isPassed/${scoreId}?isPassed=${requestValue}`, "GET");
      fetchScoreSheet();
    } catch (err) {
      toast.error("Xatolik!");
    }
  ...
  return (
    <div className="min-h-screen p-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row items-end justify-between w-full gap-4 mb-4">

          {/* 🔍 Qidiruv inputi */}
          <div className="w-full md:w-1/3">
            <label className="text-sm text-gray-600 mb-1 block">Qidiruv</label>
            <input
              type="text"
              placeholder="Talaba ismi..."
              className="px-4 py-2 border rounded-lg w-full focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition bg-gray-50"
              value={search}
          ...
          <div className="w-full md:w-1/3 flex md:justify-center">
            <div className="bg-blue-50 px-4 py-2 rounded-lg border border-blue-200 text-blue-700 font-semibold text-sm shadow-sm">
          ...
          <div className="w-full md:w-1/3 flex md:justify-end">
            <button
              ...
              className="px-5 py-2.5 rounded-lg font-semibold text-white bg-teal-600 hover:bg-teal-700 active:scale-95 transition flex items-center gap-2 shadow-md"
            >
                  ...
                  {scoreSheets.length === 0 ? (
                    ...
                    scoreSheets
                      ...
                      .map((item, index) => {
                        ...
                        console.log("Jami academic_load:", totalLoad);
                        console.log("Sababsiz NB:", sababsizNb);
                        console.log("Foiz:", percent + "%");
                        return (
                          <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-4 py-3 text-center">
                              <div className="flex items-center gap-2">
                                ...
                                {/* 1️⃣ NULL bo'lsa — Chaqirish tugmasi chiqadi */}
                                {item.getIsOffice === null && (
                                  <button
                                    ...
                                    onClick={() => {
                                      ...
                                      setOfficeModalOpen(true);   // modal ochiladi
                                    }}
                                  >
                                    Chaqirish
                                  </button>
                                ...
                                {/* 2️⃣ TRUE bo'lsa — ✔️ SVG tugmasi chiqadi */}
                                {item.getIsOffice === true && (
                                  <button
                                    className="px-3 py-1 rounded-lg text-sm text-white bg-green-600 hover:bg-green-700"
                                    onClick={() => {
                                      setSelectedScore(item);
                                      setOfficeDescription(item.officeDescription || "");
                                      updateOfficeStatus(false);  // FALSE yuboriladi
                                    }}
                                  >
                                    <svg
                                ...
                                {/* 3️⃣ FALSE bo'lsa — "Bajarildi" chiqadi */}
                                {item.getIsOffice === false && (
                                  <h3 className="text-green-700 font-medium">Bajarildi!</h3>
                                )}
                              </div>
                            </td>
                            <td className="px-3 py-3 text-center flex gap-4">
                              ...
                              {/* O‘tdi */}
                              {(item.isPassed === null || item.isPassed === true) && (
                                <button
                                  className={`px-3 py-2 rounded font-semibold text-sm transition bg-green-600 text-white border-2 border-green-800`}
                              ...
                              {(item.isPassed === null || item.isPassed === false) && (
                                <button
                                  className={`px-3 py-2 rounded font-semibold text-sm transition bg-red-600 text-white border-2 border-red-800`}
                                  onClick={() => handlePassedChange(item.id, "false")}
                                  disabled
                                >
                                  O'tmadi
                                </button>
