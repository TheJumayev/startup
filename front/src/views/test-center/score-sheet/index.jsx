...
const LS_KEY = "scoresheet_filters";

const defaultFilters = {
  group: null,
  semester: null,
  qaydnoma: null,
  search: "", // fan nomi yoki o'qituvchi nomi bo'yicha umumiy qidiruv
};
...
const ScoreSheet = () => {
  ...
  const [sheets, setSheets] = useState([]); // server dan kelgan natija
  const [hasSearched, setHasSearched] = useState(false); // birinchi qidiruv bo'ldimi?
  ...
  // ─────────────────────────────────────────
  // CLIENT-SIDE: umumiy search (fan / o'qituvchi) + qaydnoma
  // groupId + semesterCode → server tomonida filterlanadi
  // ─────────────────────────────────────────
  const filteredSheets = sheets.filter((item) => {
    const { search, qaydnoma } = activeFilters;

    // Umumiy qidiruv: fan nomi yoki seminarchi/ma'ruzachi nomi
    if (search) {
  ...
  const fetchFilteredSheets = useCallback(
    async (filters) => {
      ...
      // Guruh ham semestr ham yo'q → hech narsa qilmaymiz
  ...
  // ─────────────────────────────────────────
  // FETCH: Fanlar (curriculum bo'yicha)
  // ─────────────────────────────────────────
  ...
  // ─────────────────────────────────────────
  // INITIAL LOAD — faqat groups + teachers yuklanadi
  // sheets YUKLANMAYDI (bo'sh boshlanadi)
  // ─────────────────────────────────────────
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // LocalStorage'dan filter tiklash
        ...
        if (saved.group && saved.semester) {
          ...
          // Saqlangan filter bor bo'lsa — server so'rovi yuboriladi
          await fetchFilteredSheets(restoredFilters);
        } else {
          // Hech narsa yo'q — bo'sh holat
          setSheets([]);
  ...
  // ─────────────────────────────────────────
  const applyFilters = () => {
    // Guruh yoki semestr tanlanmagan bo'lsa — ogohlantirish
  ...
  const fetchUpdateStudents = async () => {
    try {
      setLoading(true);
      await ApiCall("/api/v1/score-sheet-group/add/update/score-sheet", "GET");
      toast.success("Talabalar yangilandi!");
    } catch {
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-8 flex flex-col items-start justify-between sm:flex-row sm:items-center">
          ...
          <div className="flex flex-wrap gap-3">
            {[
              { label: "Talabalarni yangilash", fn: fetchUpdateStudents },
              { label: "O'qituvchilar hisoboti", fn: fetchScoreStatistik },
              { label: "Talaba hisoboti", fn: fetchScoreStatistikStudent },
            ].map(({ label, fn }) => (
              <button
                key={label}
                onClick={fn}
        ...
        <div className="mb-6 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-7">
            ...
            {/* Umumiy qidiruv — fan nomi yoki o'qituvchi */}
        ...
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
          ...
          {!hasSearched ? (
            ...
            /* Holat 2: Qidiruv qilindi lekin natija yo'q */
            <div className="py-16 text-center">
              <svg
                className="mx-auto mb-4 h-16 w-16 text-gray-300"
                fill="none"
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <button
                          ...
                          onClick={() =>
                            navigate(`/test-center/vedimis/${item.id}`, {
                              state: {
                                isKursIshi: item.isKursIshi ?? false,
                                subjectName:
                                ...
                                {item.attachments?.length > 0 ? (
                                  item.attachments.map((file) => (
                                    <button
                                      ...
                                      className="flex max-w-[140px] items-center gap-1.5 rounded border border-green-200 bg-green-50 px-2.5 py-1.5 text-xs text-green-700 transition-all hover:bg-green-100"
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="flex flex-col gap-2">
                                <div className="flex gap-2">
                                  <label className="flex flex-1 cursor-pointer items-center gap-2 rounded border border-gray-300 bg-white px-3 py-2 text-xs transition-colors hover:border-blue-300 hover:bg-blue-50">
                                    <input
                                      id={`file_${item.id}`}
                                      type="file"
                                      className="hidden"
                                      onChange={(e) => {
                                        ...
                                        if (el) {
                                          ...
                                          el.className = `text-xs truncate px-1 ${
                                            f
                                    ...
                                    onClick={() => {
                                      ...
                                      if (!input?.files?.[0])
                                        return toast.warning(
                                          "⚠️ Iltimos, fayl tanlang!"
                                        );
                                      handleUpload(item.id, input.files[0]);
                                    }}
                                    className="flex items-center gap-1.5 rounded bg-blue-600 px-3 py-2 text-xs font-medium text-white transition-colors hover:bg-blue-700"
                                  >
                                    <svg
                                      className="h-3.5 w-3.5"
                                      fill="none"
                                ...
                                <div
                                  id={`file-name_${item.id}`}
                                  className="truncate px-1 text-xs text-gray-400"
                                >
                                  Hech qanday fayl tanlanmagan
                                </div>
                              </div>
