...
function GroupDetail() {
  ...
  const updateGroupStudents = async () => {
    ...
    try {
      ...
      return; // ✅ Xatolik bo'lsa funksiya to'xtaydi
    } finally {
      setLoading(false);
  ...
  const openSelectedExam = async () => {
    if (!selectedExam || pageLoading) return;
    ...
    try {
      const response = await ApiCall(
        `/api/v1/exam/start/${selectedExam}`,
        "GET",
        null,
        null,
        true
                  ...
                  {students.length > 0 ? (
                    students.map((student, index) => (
                      <tr
                        ...
                        onClick={() =>
                          !pageLoading &&
                          navigate("/admin/exams/student/" + student.id)
                        }
