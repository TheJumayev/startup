...
function ScoreStudent() {
  ...
  const isKursIshi = state?.isKursIshi ?? false;
  const subjectName = state?.subjectName ?? "";
  const [kursIshiModalOpen, setKursIshiModalOpen] = useState(false);
  const [newKursIshi, setNewKursIshi] = useState(0);
  const [selectedKursIshi, setSelectedKursIshi] = useState(null);

  const openKursIshiModal = (item) => {
    setSelectedKursIshi(item);
    setNewKursIshi(item.kursIshi ?? 0);
    setKursIshiModalOpen(true);
  };
  const updateKursIshi = async () => {
    if (!canEditScore(selectedKursIshi.scoreSheetGroup)) {
      return toast.error("Baho qo'yish vaqti tugagan!");
    }

    if (newKursIshi > 100) return toast.error("Kurs ishi 100 balldan oshmaydi!");

    const dto = {
      kursIshi: Number(newKursIshi),
      markerId: admin.id,
      qaytnoma: Number(selectedKursIshi.scoreSheetGroup.qaytnoma.split("-")[0])
    };

    try {
      await ApiCall(`/api/v1/score-sheet/${selectedKursIshi.id}`, "PUT", dto);
      toast.success("Kurs ishi bahosi saqlandi!");
      setKursIshiModalOpen(false);
      fetchScoreSheet();
    } catch (error) {
      toast.error("Xatolik!");
    }
  ...
  const openOfficeModal = (item) => {
    ...
    setOfficeDescription(item.officeDescription || ""); // ✔ to‘g‘ri bo‘ldi!
  ...
  const acceptOnlineCall = async (item) => {
    const studentId = item.student?.id;

    const dto = {
      acceptedOnlineForExam: true
    };

    await ApiCall(
      `/api/v1/score-sheet/office/${item.id}/${studentId}`,
      "PUT",
      dto
    );

    toast.success("Tanishdim belgilandi!");
    fetchScoreSheet();
  };
  ...
  const updateOffice = async () => {
    ...
    const studentId = getStudentId(selectedScore.student);
    ...
    try {
      ...
      toast.success("Talaba chaqirildi!");
  ...
  const updateOfficePermisson = async (item) => {
    if (!item) {
    ...
    try {
      const dto = {
        getIsOffice: false,       // ❗️ qayta "chaqirilmagan" holatga o'tkazadi
        officeDescription: item.officeDescription || ""
      };
  ...
  const fetchStudents = async () => {
    try {
      setLoading(true);
      const response = await ApiCall(`/api/v1/score-sheet/student/${id}`, "GET");
  ...
  const assignStudentToGroup = async () => {
    ...
    try {
      await ApiCall(
        `/api/v1/score-sheet-group/set-student/${selectedStudentId}/${groupInfo.id}`,
        "PUT"
      );

      toast.success("Talaba guruhga muvaffaqiyatli biriktirildi!");
      setAssignModalOpen(false);
      await fetchScoreSheet(); // yangilangan ma'lumotni olish
    } catch (error) {
      toast.error("Xatolik! Talaba biriktirilmadi.");
    }
  };
  ...
  return (
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            {groupInfo && (
              <div
                className="bg-white rounded-lg shadow-sm p-3 border cursor-pointer"
            ...
            <div>
              <button
                onClick={() => updateNB(id)}
                disabled={loading}
                className={`px-4 py-2 rounded-lg font-semibold text-white transition ml-4
        ...
        <div className="flex justify-between w-full">
          ...
          <div>
            <button
              ...
              className="px-4 py-2 rounded-lg font-semibold text-white bg-teal-600 hover:bg-teal-700 transition"
            >
                  ...
                  {scoreSheets.length === 0 ? (
                    ...
                    scoreSheets
                      ...
                      .map((item, index) => {
                        ...
                        const sababsizNb = item.sababsizNb;
                        const details = item.scoreSheetGroup.curriculumSubject.subjectDetails;
                          ...
                          <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-4 py-3 text-center">
                              <div className="flex items-center gap-2">
                                {(item.getIsOffice === null || item.getIsOffice === false) && (
                                  <button
                                    className="px-3 py-1 rounded-lg text-sm text-white bg-blue-600 hover:bg-blue-700"
                                    onClick={() => openOfficeModal(item)}   // ➡️ true yuboriladi
                                  >
                                    Chaqirish
                                  </button>
                                )}
                                {/* 2️⃣ Chaqirilgan holat: true */}
                                {item.getIsOffice === true && (
                                  <button
                                    className="px-3 py-1 rounded-lg text-sm text-white bg-green-600 hover:bg-green-700"
                                    onClick={() => updateOfficePermisson(item)}   // ➡️ false yuboriladi
                                  >
                                    <svg
                            ...
                            <td className="px-3 py-3 text-center">
                              <select
                                ...
                                onChange={(e) => handlePassedChange(item.id, e.target.value)}
                              >
                                <option value="null" disabled hidden>Belgilanmagan</option>
                            ...
                            {isKursIshi === true && (
                              <td className="px-3 py-3 text-center">
                                <button
                                  onClick={() => openKursIshiModal(item)}
    ...
    "            >
      ...
      <Modal
        ...
        className="bg-white rounded-xl shadow-2xl w-full max-w-md mx-auto mt-24 outline-none"
        overlayClassName="
  ...
  "
      >
        <div className="p-6">
          <h2 className="text-2xl font-bold mb-4">Talabani biriktirish</h2>

          <label className="block text-sm font-medium mb-2">
            Talaba (qidirish mumkin)
          </label>
      ...
      >
        <div className="p-8">
          <h2 className="text-2xl font-bold text-gray-800 mb-4">Talabani chaqirish</h2>
          ...
          <div className="mb-6">
            ...
            <textarea
              ...
              className="w-full border border-gray-300 rounded-lg px-4 py-3 text-lg focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
              rows="4"
            ></textarea>
          </div>
          ...
          <div className="flex justify-end space-x-4">
            <button
              className="px-5 py-2.5 border border-gray-300 text-gray-700 rounded-lg font-medium hover:bg-gray-100"
      ...
      >
        <div className="p-8">
          ...
          <p className="text-red-500 mb-5 text-xl">
            Talaba: <span className="font-semibold">{selectedKursIshi?.student?.fullName}</span>
          </p>
          ...
          <div className="mb-6">
            ...
            <input
              type="number"
              value={newKursIshi}
              onChange={(e) => setNewKursIshi(e.target.value)}
          ...
          <div className="flex justify-end space-x-4">
            <button
              ...
              onClick={() => setKursIshiModalOpen(false)}
            >
              Bekor qilish
            </button>
