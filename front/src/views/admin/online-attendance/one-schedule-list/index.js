...
/* =====================================================
   ROW COMPONENT
===================================================== */
const AttendanceRow = ({ item, index, onChange, onMarked }) => {
...
===================================================== */
const MobileAttendanceRow = ({ item, index, onChange, onMarked }) => {
  const [status, setStatus] = useState(item.isPresent ?? 0);
  const [comment, setComment] = useState(item.comment || "");
  const [file, setFile] = useState(null);
...
const Attendance = () => {
  ...
  const handleMarkedStudent = (attendanceId, isMarked) => {
  ...
  // Sort attendance list alphabetically by student full name
  ...
  // Calculate students who need to be marked - UPDATED to use todayOnline
  const unmarkedStudents = useMemo(() => {
    // Show only students with todayOnline = true from attendance list
    const onlineStudents = sortedAttendanceList.filter(
      (item) => item.todayOnline === true
    );
  ...
  // Filter students by search query - UPDATED to use todayOnline
  const visibleAttendance = useMemo(() => {
    // Show only students with todayOnline = true from attendance list
    let filtered = sortedAttendanceList.filter(
      (item) => item.todayOnline === true
    );
    ...
    return filtered;
  }, [sortedAttendanceList, searchQuery]);

  // Check if all students are marked
  const allMarked = useMemo(() => {
    if (unmarkedStudents.length === 0) return true;

    // Check if all unmarked students have been marked in current session
    const unmarkedIds = unmarkedStudents.map((item) => item.id);
    return unmarkedIds.every((id) => markedStudents.has(id));
  }, [unmarkedStudents, markedStudents]);

  const saveAll = async () => {
    // Check if all students are marked
    if (!allMarked) {
      toast.error("❌ Iltimos, barcha talabalarning davomatini belgilang!");
      return;
    }
          ...
          <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
            <div>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-4">
                <p className="text-gray-600">
                  {visibleAttendance.length} ta talaba (Online)
                </p>
                {unmarkedStudents.length > 0 && (
                  ...
                  >
                    {allMarked ? (
                      <>
                        <Check size={14} />
                        <span>Barcha talabalar belgilangan</span>
                      </>
                    ) : (
                      <>
                        <AlertTriangle size={14} />
                        <span>
                          {markedStudents.size}/{unmarkedStudents.length} talaba
                          belgilandi
                        </span>
                      </>
                    )}
            ...
            <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row">
              ...
              >
                ...
                {!allMarked
                  ? `Belgilash kerak: ${
                      unmarkedStudents.length - markedStudents.size
                    }`
        ...
        <div className="mb-6 rounded-2xl bg-white p-6 shadow-lg">
          ...
          {searchQuery && (
            <div className="mt-2 text-sm text-gray-600">
              "{searchQuery}" bo'yicha {visibleAttendance.length} ta natija
              topildi
            </div>
          )}
        </div>

        {/* Warning Alert if not all marked */}
        {!allMarked && unmarkedStudents.length > 0 && (
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="font-medium text-amber-800">
                  Iltimos, barcha talabalarning davomatini belgilang!
                </p>
                <p className="mt-1 text-sm text-amber-700">
                  {unmarkedStudents.length - markedStudents.size} ta talaba hali
                  belgilanmagan. Saqlash tugmasi faqat barcha talabalar
                  belgilangandan so'ng ishlaydi.
                </p>
              </div>
            </div>
          </div>
              ...
              {visibleAttendance.length === 0 ? (
                <div className="rounded-lg border border-dashed border-gray-300 p-8 text-center">
                  ...
                  <p className="mt-4 text-gray-500">
                    {searchQuery
                      ? "Qidiruv natijasi bo'yicha talaba topilmadi"
                      : "Online talabalar ro'yxati bo'sh"}
                  </p>
                </div>
              ) : (
          ...
          {visibleAttendance.length > 0 && !isMobile && (
            <div className="border-t bg-gray-50 px-6 py-4">
              <div className="flex items-center justify-between">
                  ...
                  {unmarkedStudents.length > 0 && (
                    <p
                      className={`font-medium ${
                        allMarked ? "text-green-600" : "text-amber-600"
                      }`}
                    >
                      {allMarked
                        ? "✅ Barchasi belgilandi"
                        : `🔄 ${markedStudents.size}/${unmarkedStudents.length} belgilandi`}
                    </p>
                  )}
                </div>
                <button
                ...
                >
                  <Save size={18} />
                  {!allMarked ? "Barchasini belgilang" : "Hammasini saqlash"}
                </button>
              </div>
            </div>
          )}
