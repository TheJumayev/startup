...
const ScoreSheet = () => {
  ...
  const [isKursIshi, setIsKursIshi] = useState(false);
  ...
  const fetchScoreStatistikStudent = async () => {
    try {
      ...
      const response = await fetch(`${baseUrl}/api/v1/score-sheet/excel`, {
        method: "GET",
        headers: {
      ...
      // Excel fayl blob sifatida keladi
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
          ...
          <div className="flex gap-4">
            ...
            >
              <span>O'qituvchilar hisoboti</span>
            </button>
            <button
              onClick={() => fetchScoreStatistikStudent()}
              className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-medium transition-colors duration-200 flex items-center space-x-2 shadow-sm"
            >
              <span>Talaba hisoboti</span>
            </button>
            <button
              onClick={() => {
                clearForm();
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-200">
          {sheets.length === 0 ? (
            <div className="text-center py-12">
              ...
              <button
                onClick={() => {
                  clearForm();        // 🟢 barcha inputlarni tozalaydi
                  setIsKursIshi(false); // 🟢 kurs ishi checkboxini ham tozalash
                  setSelectedQaydnoma(null); // 🟢 qaydnomani ham tozalash
                  setCreateModalOpen(true);   // 🟢 modalni ochish
                }}
                    ...
                    <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="text-sm font-medium text-gray-900">
                          <button
                            ...
                            onClick={() =>
                              navigate(`/dekan/schedule/${item.id}`, {
                                state: {
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex space-x-2">
                          <button
                            className="px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg font-medium text-sm transition-colors duration-200 flex items-center space-x-1"
                            onClick={() => {
                              setEditItem(item);
                              ...
                              setIsKursIshi(item.isKursIshi ?? false);
                              setSelectedTeacher({
                                value: item.teacher.id,
                                label: item.teacher.name,
                              });

                              // 1) fanlarni yuklaymiz
                              fetchSubjects(item.group.curriculum);

                              // 2) subject yuklanishini kutib, so‘ng set qilamiz
                              setTimeout(() => {
                                setSelectedSubject({
                          ...
                          <button
                            className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg font-medium text-sm transition-colors duration-200 flex items-center space-x-1"
                            onClick={() => {
                          ...
                          <label style={{ position: "relative", display: "inline-block", width: "60px", height: "34px", cursor: "pointer" }}>
                            <input
                              type="checkbox"
                              checked={item.status}
                              onChange={() => handleStatusClick(item.id)}   // 🔥 Modal ochiladi
                            ...
                            <span
                              style={{
                                ...
                                right: 0,
                                bottom: 0,
                                backgroundColor: item.status ? "#2196F3" : "#ccc",
                                transition: ".4s",
                                borderRadius: "34px",
                              }}
                            ></span>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 mt-2">
            <input
              type="checkbox"
              checked={isKursIshi}
              onChange={(e) => setIsKursIshi(e.target.checked)}
              className="w-5 h-5"
            />
