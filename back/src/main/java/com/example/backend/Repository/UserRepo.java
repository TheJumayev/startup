...
public interface UserRepo extends JpaRepository<User, UUID> {
    ...
    @Query(value = "SELECT u.* FROM users u JOIN users_roles ur ON u.id = ur.user_id JOIN role r ON ur.roles_id = r.id WHERE r.name = 'ROLE_BUGALTER'", nativeQuery = true)
    List<User> findAllBankerByRole();
    @Query(value = "SELECT u.* FROM users u JOIN users_roles ur ON u.id = ur.user_id JOIN role r ON ur.roles_id = r.id WHERE r.name = 'ROLE_SECRETARY'", nativeQuery = true)
    List<User> findAllSecretaryByRole();

    @Query(value = "SELECT u.* FROM users u JOIN users_roles ur ON u.id = ur.user_id JOIN role r ON ur.roles_id = r.id WHERE r.name = 'ROLE_TEST_CENTER'", nativeQuery = true)
    List<User> findAllTestCenterByRole();
