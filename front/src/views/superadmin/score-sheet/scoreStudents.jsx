...
function ScoreStudent() {
  ...
  const [rektorModalOpen, setRektorModalOpen] = useState(false);
  const [rektorStatus, setRektorStatus] = useState("1"); // 1 o'tdi, 0 o'tmadi
  const [rektorDescription, setRektorDescription] = useState("");
  const [selectedRektorScoreId, setSelectedRektorScoreId] = useState(null);
  const [extraModalOpen, setExtraModalOpen] = useState(false);
  const [newExtraScore, setNewExtraScore] = useState(0);
  const [selectedExtra, setSelectedExtra] = useState(null);
  const { state } = useLocation(); // navigate orqali yuborilgan data
  ...
  const saveRektorStatus = async () => {
    if (!selectedRektorScoreId) return;

    try {
      const url = `/api/v1/score-sheet/rektor/${selectedRektorScoreId}/${rektorStatus}/${rektorDescription}`;
      await ApiCall(url, "GET");
      toast.success("Rektor qarori saqlandi!");

      setRektorModalOpen(false);
      setRektorDescription("");
      setRektorStatus("1");

      fetchScoreSheet(); // UI yangilash
    } catch (e) {
      toast.error("Xatolik!");
    }
  };
  ...
  const deleteStudent = async (id) => {
    if (!window.confirm("Ushbu talabani o'chirmoqchimisiz?")) return;

    try {
      await ApiCall(`/api/v1/score-sheet/delete/${id}`, "DELETE");

      toast.success("Talaba muvaffaqiyatli o'chirildi!");
      fetchScoreSheet(); // UI-ni yangilash
    } catch (error) {
      console.error(error);
      toast.error("Talabani o'chirishda xatolik!");
    }
  };
  ...
  const handlePassedChange = async (scoreId, value) => {
    // value = "true" → O'tmadi → backendga false
    // value = "false" → O'tdi → backendga true

    let requestValue = null;

    if (value === "true") requestValue = false; // O'tmadi
    if (value === "false") requestValue = true; // O'tdi
  ...
  return (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-center">
                              <div className="flex items-center gap-2">
                                {(item.getIsOffice === null ||
                                  item.getIsOffice === false) && (
                                  <button
                            ...
                            <td className="px-3 py-3 text-center">
                              <select
                                ...
                                value={
                                  item.isPassed === null
                                    ? "null"
                                    : item.isPassed === true
                                    ? "false" // O'tdi → selectda false bo'lishi kerak
                                    : "true" // O'tmadi → selectda true bo'ladi
                                }
                                onChange={(e) =>
                              ...
                              >
                                ...
                                <option value="false">O'tdi</option>
                                <option value="true">O'tmadi</option>
                              </select>
                            </td>
                            {/* Mustaqil ta'lim */}
                            <td className="px-3 py-3 text-center">
    ...
    `}
                            ...
                            <td className="px-3 py-3 text-center">
                              <button
                                onClick={() => {
                                  setSelectedRektorScoreId(item.id);
                                  setRektorDescription(
                                    item.rektorDescription || ""
                                  );
                                  setRektorStatus(item.rektor ? "1" : "0");
                                  setRektorModalOpen(true);
                                }}
                                className="rounded-lg bg-blue-600 px-3 py-1 text-sm text-white hover:bg-blue-700"
                              >
                                Rektor
    ...
    "
      ...
      <Modal
        isOpen={rektorModalOpen}
        onRequestClose={() => setRektorModalOpen(false)}
        className="mx-auto mt-24 w-full max-w-md rounded-xl bg-white shadow-2xl outline-none"
        overlayClassName="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-start pt-20 z-50"
      >
        <div className="p-6">
          <h2 className="mb-4 text-xl font-bold">Rektor tasdig'i</h2>

          <label className="mb-1 block font-medium">Holat</label>
          <select
            value={rektorStatus}
            onChange={(e) => setRektorStatus(e.target.value)}
            className="mb-4 w-full rounded-lg border p-2"
          >
            <option value="1">O'tmadi</option>
            <option value="0">O'tdi</option>
          </select>

          <label className="mb-1 block font-medium">Izoh</label>
          <textarea
            rows={3}
            className="w-full rounded-lg border p-2"
            placeholder="Izoh kiriting..."
            value={rektorDescription}
            onChange={(e) => setRektorDescription(e.target.value)}
          ></textarea>

          <div className="mt-5 flex justify-end gap-3">
            <button
              onClick={() => setRektorModalOpen(false)}
  ...
  "
      >
        <div className="p-6">
          <h2 className="mb-4 text-2xl font-bold">Talabani biriktirish</h2>
