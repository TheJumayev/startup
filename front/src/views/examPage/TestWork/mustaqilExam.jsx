...
export default function TestSolveAll() {
  ...
  const fetchSubjects = async () => {
    try {
      const res = await ApiCall(
        `/api/v1/mustaqil-exam-student/one/${id}`,
        "GET"
      );
      console.log(res.data);

      setExamSubject(res.data);
      const exam = res.data.mustaqilExam;
  ...
  const loadQuestions = async () => {
    try {
      const res = await ApiCall(
        `/api/v1/mustaqil-exam-student/start-test/${id}`,
        "GET"
      );
  ...
  const handleSelect = async (testId, optionIndex) => {
    ...
    try {
      await ApiCall(
        `/api/v1/mustaqil-exam-student-test/${testId}/${optionIndex}`,
        "GET"
      );
    } catch (err) {
  ...
  const finishTest = async () => {
    ...
    try {
      const res = await ApiCall(
        `/api/v1/mustaqil-exam-student-test/finish-exam/${id}`,
        "GET"
      );
    ...
    <div className="relative flex min-h-screen select-none gap-6 bg-gray-100 py-6 px-4">
      ...
      <div className="fixed left-4 top-4 h-[calc(100vh-2rem)] w-64 overflow-y-auto rounded-xl border bg-white p-4 shadow-xl">
        ...
        <div className="space-y-2 text-sm">
          ...
          <p>
            <b>Fan nomi:</b> {examSubject?.mustaqilExam?.name}
          </p>
          <p>
            <b>Urunishlar soni:</b> {examSubject?.attempt}/
            {examSubject?.mustaqilExam?.attempts}
          </p>
        </div>
