...
const Duty = () => {
  ...
  const [endErkinDate, setEndErkinDate] = useState("");
  ...
  const confirmAction = async () => {
    ...
    setConfirming(true);
    if (action === "online" && !endErkinDate) {
      toast.error("Online tugash sanasini tanlang ❌");
      setConfirming(false);
      return;
    }
    ...
    try {
      ...
      if (action === "online") {
        ...
        await ApiCall(`/api/v1/student/erkin/${selectedStudent.id}`, "PUT", {
          endErkinJadval: endErkinDate,
        });
      } else {
        res = await ApiCall(
  ...
  return (
    <div className="mx-auto max-w-7xl p-6">
      <ToastContainer />
      <div>
        <Breadcrumbs items={[{ label: "Guruhlar", to: "/admin/groups" }]} />
      </div>
                ...
                {filteredStudents.map((student, index) => (
                  ...
                  >
                    <td
                      ...
                      onClick={() =>
                        navigate(`/admin/groups/group/${student.id}`, {
            ...
            {action === "online" && (
              <div className="mb-4">
                <label className="mb-1 block text-sm font-medium text-gray-700">
                  Online tugash sanasi
                </label>
                <input
                  type="date"
                  value={endErkinDate}
                  onChange={(e) => setEndErkinDate(e.target.value)}
                  className="w-full rounded border px-3 py-2 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                />
              </div>
            )}
