...
const Teacher = () => {
  ...
  const [loadingUpdateBase, setLoadingUpdateBase] = useState(false);
  const [loadingFromHemis, setLoadingFromHemis] = useState(false);
  ...
  const deleteAssignedRow = async (id) => {
    ...
    try {
      await ApiCall(`/api/v1/teacher-curriculum-subject/${id}`, "DELETE");
  ...
  const updateAssignedTeacher = async () => {
    ...
    try {
      await ApiCall(`/api/v1/teacher-curriculum-subject/${editOldTeacher.id}/${editNewTeacher.value}`, "PUT");
  ...
  const updateBaseFromBackend = async () => {
    setLoadingUpdateBase(true);
    try {
      const res = await ApiCall("/api/v1/teacher/hemis-teacher-update", "GET");
      toast.success("Baza yangilandi!");
      await getTeachers();
    } catch (error) {
      console.error(error);
      toast.error("Yangilashda xatolik!");
    } finally {
      setLoadingUpdateBase(false);
    }
  };

  const fetchFromHemis = async () => {
    setLoadingFromHemis(true);
    try {
      const res = await ApiCall("/api/v1/hemis-teacher/update", "GET");
      toast.success("Hemisdan olingan ma’lumotlar yuklandi!");
      await getTeachers();
    } catch (error) {
      console.error(error);
      toast.error("Hemisdan olishda xatolik!");
    } finally {
      setLoadingFromHemis(false);
    }
  };
  ...
  return (
    <div className="min-h-screen p-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-800">O'qituvchilar boshqaruvi</h1>
        ...
        {/* BAZANI YANGILASH */}
        <button
          onClick={updateBaseFromBackend}
          disabled={loadingUpdateBase}
          className={`flex items-center gap-2 rounded-lg px-4 py-2 text-white transition-colors ${loadingUpdateBase
        ...
        >
          {loadingUpdateBase ? (
            ...
            <>
              <FiPlus /> O'qtuvchi rolini berish
            </>
          )}
        </button>
        ...
        {/* HEMISDAN OLISH */}
        <button
          onClick={fetchFromHemis}
          disabled={loadingFromHemis}
          className={`flex items-center gap-2 rounded-lg px-4 py-2 text-white transition-colors ${loadingFromHemis
            ? "bg-blue-400 cursor-not-allowed"
            : "bg-blue-600 hover:bg-blue-700"
            }`}
        >
