...
function Index() {
  ...
  const filteredTeachers = teachers.filter((t) => {
    ...
    // 🔹 Yo‘nalish filtri (supervisors asosida)
    const matchDirection = filterDirection
      ? supervisors.some(
        (s) =>
          s.group.toLowerCase().includes(filterDirection.toLowerCase()) &&
          s.name.trim().toLowerCase() ===
          (t.teacherName?.trim().toLowerCase() || "")
      )
      : true;

    return matchGroup && matchSearch && matchDirection;
  });
  ...
  // 🆕 Kontrakt summasini olish funksiyasi
    ...
    <div className="min-h-screen p-4 md:p-6">
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-6">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              const found = supervisors.find((s) => s.group === selected);
        ...
        <div className="overflow-hidden rounded-lg bg-white shadow">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-gray-200 bg-gradient-to-r from-indigo-50 via-white to-indigo-50 px-6 py-5 rounded-t-xl shadow-sm transition-all">
            <div>
              <h2 className="text-xl font-semibold text-gray-900 tracking-tight">
                {filterGroup
                  ? `Magistr mavzular va ilmiy rahbarlar (${filterGroup} guruhi)`
                  : "Magistr mavzular va ilmiy rahbarlar"}
              </h2>

              {/* 🆕 Rahbar ma'lumoti chiroyli badge ko‘rinishida */}
              {selectedSupervisor && (
                <div className="mt-2 flex items-center gap-3">
                  <div className="flex items-center bg-indigo-100 text-indigo-800 px-3 py-1 rounded-full text-sm font-medium shadow-sm">
                    <span className="mr-1.5">👤</span>
                    {selectedSupervisor.name}
                  </div>
                  <div className="flex items-center bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm font-medium shadow-sm">
                    <span className="mr-1.5">📞</span>
            ...
            <span className="mt-3 sm:mt-0 text-base font-semibold text-gray-700 bg-gray-100 px-4 py-1.5 rounded-lg shadow-sm">
              Jami:{" "}
                ...
                {filteredTeachers.length > 0 ? (
                  filteredTeachers.map((t, idx) => (
                    <tr key={t.id} className="hover:bg-gray-50">
                      ...
                      <td className="whitespace-nowrap px-3 py-2 text-xs text-gray-700 font-semibold">
                        {t.contract ? t.contract.payment?.toLocaleString("uz-UZ") + " so‘m" : "-"}
                      </td>
