      ...
      if (curriculumsResponse?.data) {
        ...
        setSelectOptions((prev) => ({
          ...
          curriculums: curriculums.map((curriculum) => ({
            value: curriculum.id,
            label: `${curriculum.educationYearName || "Noma'lum yil"} - ${
              curriculum.specialty?.name || "Noma'lum mutaxassislik"
              ...
              <div className="w-full md:w-1/3">
                ...
                <Select
                  ...
                  onChange={(selected) => {
                    ...
                    if (!selectedId) {
                      // если предмет очищен — показать все
                      const mapped = originalCurriculums.map((item) => {
                        const cs = item.subject;
                        const subj = cs?.subject || {};
                        return {
...
const TableRow = ({ curriculum, navigate, index }) => (
  <tr
    ...
    onClick={() => navigate(`/admin/curriculum-subject/${curriculum.id}`)}
  >
    ...
    >
      <button
        onClick={() =>
          navigate(`/admin/test-upload/${curriculum.id}`, {
...
const PaginationButton = ({
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`relative inline-flex items-center border px-3 py-2 text-sm font-medium focus:z-20 ${
        active
