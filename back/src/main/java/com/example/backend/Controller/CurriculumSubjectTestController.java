...
import com.example.backend.DTO.CurriculumSubjectTestDTO;
...
@CrossOrigin
@RestController
@RequestMapping("/api/v1/test-curriculum-subject")
@RequiredArgsConstructor
public class CurriculumSubjectTestController {
    ...
    public ResponseEntity<?> getTestFormSubject(@PathVariable UUID curriculumSubjectId) {
        ...
        return ResponseEntity.ok(pickRandomUpTo(tests, 200));
    }
    @GetMapping("/test-student/{curriculumSubjectId}")
    public ResponseEntity<?> getTestFormSubjectStudent(@PathVariable UUID curriculumSubjectId) {
        List<TestCurriculumSubject> tests = testCurriculumSubjectRepo.findByCurriculumSubjectId(curriculumSubjectId);
    ...
    @PutMapping("/test-one/{testId}")
    public ResponseEntity<?>editTestForm(@PathVariable UUID testId, @RequestBody CurriculumSubjectTestDTO dto) {
        Optional<TestCurriculumSubject> test = testCurriculumSubjectRepo.findById(testId);
        if (test.isEmpty()) {
            return ResponseEntity.notFound().build();
        }
        TestCurriculumSubject testCurriculumSubject = test.get();
        testCurriculumSubject.setQuestion(dto.getQuestion());
        testCurriculumSubject.setAnswer1(dto.getCorrectAnswer());
        testCurriculumSubject.setAnswer2(dto.getAnswer1());
        testCurriculumSubject.setAnswer3(dto.getAnswer2());
        testCurriculumSubject.setAnswer4(dto.getAnswer3());
        TestCurriculumSubject save = testCurriculumSubjectRepo.save(testCurriculumSubject);
