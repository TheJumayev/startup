...
/* =====================================================
   STUDENT TYPE BADGE
===================================================== */
const StudentTypeBadge = ({ isOnline }) => {
  return (
    <div
        ...
        <div className="flex items-center gap-3">
          <div
            className={`flex h-8 w-8 items-center justify-center rounded-full ${
              item.student?.isOnline ? "bg-blue-100" : "bg-gray-100"
            }`}
          >
            <img className={"rounded-full"} src={item.student.image} />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <p className="font-medium text-gray-900">{fullName}</p>
              <StudentTypeBadge isOnline={item.student?.isOnline} />
            </div>
      ...
      <td className="px-4 py-4">
        {item?.file?.id ? (
          <button
            ...
            className="coursor-pointer inline-flex items-center gap-1 text-sm text-blue-600 hover:text-blue-700 hover:underline"
          >
            <Download size={14} />
            Yuklab olish
          </button>
          ...
          >
            <span
              className={`text-sm font-medium ${
                item.student?.isOnline ? "text-blue-600" : "text-gray-600"
              }`}
            >
              {index + 1}
            </span>
...
const Attendance = () => {
  ...
  const stats = useMemo(() => {
    const online = attendanceList?.filter((item) => item.student?.isOnline);
    const offline = attendanceList?.filter((item) => !item.student?.isOnline);
  ...
  const filteredAttendance = useMemo(() => {
    ...
    if (filterType === "online") {
      result = result.filter((item) => item.student?.isOnline);
